...
result * twoadd(CLIENT *clnt, int x, int y) {
   operands ops;
   result *ret;
...
result * twosub(CLIENT *clnt, int x, int y) {
   operands ops;
   result *ret;
...
int main( int argc, char *argv[]) {
   ...
   printf("Using the secondary addition and subtraction...\n");
   ret = twoadd(clnt, x, y);
   printf("%d + %d = %d\n", x, y, ret->r);
   ret = twosub(clnt, x, y);
   printf("%d - %d = %d\n", x, y, ret->r);
