...
/// \param [in] filename imports the name of the file to open
/// \throw invalid_file if the file can not be opened
/// \post \a iStr is attached to a valid file, ready for extraction
//**************************************************************************
void openIn(ifstream& iStr,const string& filename) throw (invalid_file) {
   iStr.open(filename.c_str());
   if (!iStr) throw(invalid_file(filename+" can not be opened"));
}
